...
def agent_prompt(layer_name: str, layer: dict, repo_map: dict,
    ...
    """
    tech = ", ".join(repo_map.get("tech_stack", []))
    other_agents = [f"`{l}-agent`" for l in all_layers if l != layer_name]
...
- Sub-agents list must name: {", ".join(f"`{l}-agent`" for l in layers)}
...
def build_skill_registry(
    ...
    lines.append(f"- **Generated by**: repoforge\n")

    return "".join(lines)


# Keep Path import available for build_skill_registry
